...
1. 为本地仓库创建一个远程的GitHub仓库
   1. 在GitHub网页上new一个仓库
   2. 通过`git remote add origin ``git@github.com``:lyydsheep/learn_gogogogo.git`命令可以将本地仓库和远程仓库进行关联
   3. 使用`git remote`可以查看远程仓库的名称
2. 配置公私钥，获取`push`远程仓库的权限
   1. 使用`ssh-keygen -t rsa -C "2230561977@qq.com"`命令生成公私钥
   2. 将公钥的具体内容复制到`ssh and GPG keys`模块中
